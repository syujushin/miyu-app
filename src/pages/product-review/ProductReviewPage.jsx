...
const REVIEWS_MATCHED = [
  {
    ...
    tags: ['복합성', '민감성', '봄웜', '6개월 사용'], rating: 5, image: imgReview04,
  ...
  },
  {
    ...
    tags: ['건성', '봄웜', '모공', '1개월 사용'], rating: 4, image: imgReview02,
  ...
  },
  {
    ...
    tags: ['건성', '민감성', '봄웜', '2주 사용'], rating: 5, image: imgReview03,
...
function StarRow({ rating, size = 13, fillColor = '#C0A3F7', emptyColor = '#DAD8DE' }) {
  return (
    <div style={{ display: 'flex', gap: 1 }}>
      {[1,2,3,4,5].map(i => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24">
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"
            fill={i <= rating ? fillColor : emptyColor} />
        </svg>
      ))}
    </div>
  )
...
export default function ProductReviewPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#FFFFFF', fontFamily: "'SUIT', sans-serif", letterSpacing: '-0.01em' }}>
      ...
      {/* 헤더 */}
      <div style={{ flexShrink: 0, borderBottom: '1px solid #F0EFF3' }}>
        {/* Row 1: 뒤로가기 + 타이틀 (최상단에서 58px) */}
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: 11, paddingBottom: 10, paddingLeft: 16, paddingRight: 16 }}>
          <button
            onClick={() => navigate(-1)}
          ...
          <p style={{ fontSize: 18, fontWeight: 500, color: '#242227', margin: 0, lineHeight: 1.4 }}>제품리뷰</p>
        </div>
        {/* Row 2: 내 피부 맞춤 + 토글 (좌) / 필터 (우) */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 16, paddingRight: 16, paddingBottom: 10 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {/* AI 분석 카드 */}
        <div style={{ backgroundColor: '#F6F2FF', borderRadius: 16, margin: 16, marginBottom: 12, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
            <span style={{ fontSize: 16 }}>🔮</span>
            <span style={{ fontSize: 15, fontWeight: 700, color: '#242227', lineHeight: 1.4 }}>
          ...
          {skinMatchOn ? (
            <>
              <p style={{ fontSize: 13, color: '#5F5C66', lineHeight: 1.65, margin: '0 0 8px' }}>
                구르님 피부에는 <span style={{ color: '#6633CC', fontWeight: 600 }}>데일리 진정·속보습용 세럼</span>으로 적합해요. 꾸준히 사용했을 때 피부가 한결 편안해졌다는 후기가 많고, 자극 없이 사용 가능해 민감한 날에도 부담이 적어요.
              </p>
              <p style={{ fontSize: 13, fontWeight: 500, color: '#6633CC', lineHeight: 1.65, margin: 0 }}>
                가벼운 제형으로 메이크업 전에 잘 어울려요. 화장이 밀리지 않고 베이스가 잘 먹는다는 평가가 많아요.
              </p>
            </>
          ) : (
            <>
              <p style={{ fontSize: 13, color: '#5F5C66', lineHeight: 1.65, margin: '0 0 8px' }}>
                전반적으로 <span style={{ color: '#6633CC', fontWeight: 600 }}>무겁지 않은데 촉촉하다</span>는 평이 많아요. 리치한 제형을 꺼리는 지성 피부와, 속건조를 잡고 싶어하는 건성 피부 모두를 만족시킨 황금 밸런스라는 평이에요.
              </p>
              <p style={{ fontSize: 13, color: '#5F5C66', lineHeight: 1.65, margin: 0 }}>
                특히 예민한 피부를 가진 분들도 트러블 반응 없이 한 통을 다 비웠다는 <span style={{ color: '#6633CC', fontWeight: 600 }}>순한 사용감</span>에 만족감을 드러내고 있어요.
              </p>
            </>
          )}
        </div>

        {/* 평점 + 별 + 사진 썸네일 (별도 하단 배치) */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, margin: '0 16px 12px' }}>
          <div style={{ flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 2, marginBottom: 5 }}>
              <span style={{ fontSize: 28, fontWeight: 700, color: '#6633CC', lineHeight: 1 }}>4.5</span>
              <span style={{ fontSize: 13, color: '#5F5C66', lineHeight: 1 }}>점</span>
            </div>
            <StarRow rating={4} size={13} />
          </div>
          <div style={{ display: 'flex', gap: 4 }}>
            {[imgReview01, imgReview02, imgReview03].map((img, i) => (
              <div key={i} style={{ width: 64, height: 64, borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
                <img src={img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              </div>
            ))}
            <div style={{ width: 64, height: 64, borderRadius: 8, overflow: 'hidden', flexShrink: 0, position: 'relative' }}>
              <img src={imgReview04} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 11, fontWeight: 500, color: '#FFFFFF' }}>더보기</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* LV 배너 */}
        <div style={{ backgroundColor: '#6633CC', borderRadius: 8, margin: '0 16px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 13 }}>💎</span>
          <p style={{ fontSize: 12, fontWeight: 500, color: '#FFFFFF', margin: 0 }}>LV. 리뷰어 등급은 평가 지수를 종합한 활동 데이터입니다</p>
        </div>
        ...
        <div>
          {currentReviews.map(r => (
            <div key={r.id}>
              <div style={{ height: 1, backgroundColor: '#F4F3F7', marginTop: 20 }} />
      ...
      {/* 하단 구매 바 */}
      <div style={{ backgroundColor: '#FFFFFF', paddingTop: 16, paddingBottom: 32, paddingLeft: 16, paddingRight: 16, borderTop: '1px solid #F0EFF3', flexShrink: 0 }}>
